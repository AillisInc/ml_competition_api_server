...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        metrics_list = Parallel(n_jobs=-1)([delayed(self.calculate_metrics_in_each_class)
                                            (iou_threshold=IOUThreshold,
                                             class_name=c,
                                             detections=detections,
                                             groundTruths=groundTruths,
                                             method=method) for c in classes])
        # metrics_list = []
        # for c in classes:
        #     r = self.calculate_metrics_in_each_class(iou_threshold=IOUThreshold, class_name=c,
        #                                              detections=detections, groundTruths=groundTruths, method=method)
        #     metrics_list.append(r)
        return metrics_list

    def calculate_metrics_in_each_class(self, iou_threshold, class_name, detections, groundTruths, method):
        # Get only detection of class c
        dects = []
        [dects.append(d) for d in detections if d[1] == class_name]
        # Get only ground truths of class c
        gts = []
        [gts.append(g) for g in groundTruths if g[1] == class_name]
        npos = len(gts)
        ...
        acc_TP = np.cumsum(TP)
        rec = acc_TP / npos
        average_rec = mean(rec) if len(rec) > 0 else 0
